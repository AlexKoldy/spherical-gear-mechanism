...
class Mechanism():
	...
	def inverse_kinematics(self, q_cs_des):
		...
		self.beta_1 = [theta_A1, theta_A2, theta_B1, theta_B2]
		large_bevel_t= 50
		small_bevel_t = 14
		self.alpha_1 = self.beta_1[1] + ((self.beta_1[0]*small_bevel_t)/large_bevel_t)     #MP-gear position
		self.alpha_2 = self.beta_1[3]- ((self.beta_1[2]*small_bevel_t)/large_bevel_t)       #MP-gear position
		self.alpha_3 = self.beta_1[1] + ((self.beta_1[0]*small_bevel_t)/large_bevel_t)     #MP-gear position
		self.alpha_4 = self.beta_1[3]- ((self.beta_1[2]*small_bevel_t)/large_bevel_t)       #MP-gear position



		self.q_mp_A = np.array([[self.beta_1[0]], # roll [rad]
									 [-2*self.beta_1[1]]]) # pitch [rad]
		self.q_mp_B = np.array([[self.beta_1[2]], # roll [rad]
									 [-2*self.beta_1[3]]]) # pitch [rad]

		q_differential_A = np.array([[self.alpha_1], 
									 [self.alpha_2]]) 
		q_differential_B = np.array([[self.alpha_3], 
									 [self.alpha_4]]) 
